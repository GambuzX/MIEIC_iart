...
def apply_dfs(init_state, ops, max_depth):

    # put valid operations from the initial state in the stack
    stack = []
    seen = set()
    for op in ops:
        if op(copy_state(init_state)):
            stack.append((copy_state(init_state), op, [], 0))
            seen.add(get_state_str(init_state, op))
    
    while not len(stack) == 0:
        # get next operation
        curr_state, op, previous_ops, depth = stack.pop()
        ...
        # reached max depth, abort branch
        if depth == max_depth:
            continue
        ...
        for op in ops:
            ...
            if test:
                stack.append((copy_state(curr_state), op, op_sequence, depth+1))
    return False

depth = 1
while not apply_dfs(initial_state, operators, depth):
    depth += 1